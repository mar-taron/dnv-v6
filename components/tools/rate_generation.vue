			...
			<div class="white_pad table_wrap">
				...
				<div class="table-responsive">
					<table class="table table-striped table-hover carrier_groups">
					  <thead>
						<tr>
						  <th class="table_column" @click="sortColumn('name')" v-if="showcolumns[0]">Rate Template<span class="dnl_icon dnl_sort pull-right"></span></th>
						  <th class="table_column" @click="sortColumn('create_on')" v-if="showcolumns[1]">Created on<span class="dnl_icon dnl_sort pull-right"></span></th>
                          <th class="table_column" @click="sortColumn('create_by')" v-if="showcolumns[2]">Created by<span class="dnl_icon dnl_sort pull-right"></span></th>
                          <th class="table_column" @click="sortColumn('last_generated')" v-if="showcolumns[3]">Last Generated<span class="dnl_icon dnl_sort pull-right"></span></th>
					  ...
					  <tbody>
						...
						<tr v-for="rate in rates">
						  ...
						  <td v-if="showcolumns[4]">
                              <a class="action action_money" data-toggle="tooltip" data-placement="top" title="Generation" @click="askGenerateConfirm(rate.id)">
	...
	module.exports = {
		...
		methods: {
			...
			getGenerateRateForRequest() {
				...
				// "default_rate": 0,
				// "rate_table_type": "A-Z",
				// "last_generated": "2017-09-18T07:33:36.082Z",
				// "name": "string",
				// "default_min_time": 0
			},
			generateRate(id) {
				// const url = api.getEndpointUrl() + '/v1/tool/rate_generation/';
				// const reqBody = this.getGenerateRateForRequest()
				// this.$http.post(url, reqBody)
				// .then(response => {
				// 	if (response.body.success) {
				// 		this.setMessage('Rate was generated successfully');
				// 		var thisvm = this;
				// 		setTimeout(function(){
				...
				// 	console.log(error)
				// 	this.setMessage({
				// 		message: 'Failed to generate rate',
				// 		type: 'error'
				// 	})
				// })	
			},
			askGenerateConfirm(id) {
				this.setAsyncConfirm('Are you sure to generation?')
				.then(result => {
					if (result.accepted) {
						this.generateRate(id)
					}
				})
				.catch(error => {
					console.log(error)
