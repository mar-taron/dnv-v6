	...
	<div class="container-fluid">
      <div class="row">
		<div class="wrapper single">
			...
			<h1 class="page-header">Create New Rate Generation</h1>
...
<script>   
	...
	module.exports = {
        ...
        methods: {
            ...
            resetRate () {
                this.rate = JSON.parse( JSON.stringify( this.emptyRate ) )
            },
            getRateForRequest() {
                var vm = this;
                vm.rate.egress_str = "";
                ...
                const rate_template = JSON.parse( JSON.stringify( this.rate_template ) )
			    ...
			    this.$validator.validateAll().then(() => {
                    ...
                    if (err.errors.length > 0 || vm.rate.vendors.length == 0) {
                        ...
                        if(err.errors.length > 0) {
                            msg = err.errors[0].field + ' field can not leave as null';
                        }
            ...
            fetchVendors() {
                ...
                .then(response => {
                    const body = response.body
                    if (body.success) {
                        const vendor_list = body.payload.items
                        var self = this;
                        vendor_list.forEach(function (temp, i) {
                            var vendor = {};
                            vendor.key = temp.client_id;
                            vendor.label = temp.name;
