                        ...
                        <table class="table table-striped table-hover aliases">
                            <thead>
                                <tr>
                                    ...
                                    <th v-if="showcolumns[0]" @click="sortColumn('digit_map_name')">Name
                                        <span class="dnl_icon dnl_sort pull-right"></span>
                                    </th>
                                    <th v-if="showcolumns[1]" @click="sortColumn('digit_map_count')">Digital Map Count
                                        <span class="dnl_icon dnl_sort pull-right"></span>
                                    </th>
                                    <th v-if="showcolumns[2]" @click="sortColumn('updated_at')">Updated at
                                        <span class="dnl_icon dnl_sort pull-right"></span>
                                    </th>
...
module.exports = {
    ...
    data: function () {
        return {
            ...
            map_table_columns: ['Name', 'Digital Map Count', 'Updated at', 'Action'],
    ...
    methods: {
			...
			handleChange() {
				...
				if (_.indexOf(this.map_table_columns, 'Updated at') != -1) {
        ...
        fetchMaps() {
            ...
            this.loading = true;
            this.$http.get(api.getEndpointUrl() + "/v1/route/digit_map/list" + this.getQueries(),
                {
                    headers: {
                        "X-Auth-Token": auth.getToken()
                    }
