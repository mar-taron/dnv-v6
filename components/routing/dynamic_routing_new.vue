				...
				<div class="white_pad">
					...
					<form class="form">
						...
						<div role="group" class="btn-group"> 
							...
							<button type="button" class="btn btn-default" @click="addAllEgress">
...
<script>  
	...
	module.exports = {
		...
		methods: {
			...
			fetchEgressTrunks() {
				var per_page=1;
				var url = api.getEndpointUrl() + "/v1/trunk/egress/list?per_page="+per_page
				this.loading = true;
				this.$http.get(url)
				.then(response => {
					...
					if (body.success) {
						...
						url = api.getEndpointUrl() + "/v1/trunk/egress/list?per_page="+per_page
						this.$http.get(url)
						.then(response => {
							const body = response.body
							if (body.success) {
								this.egress_trunk_list = body.payload.items;
								console.log(this.egress_trunk_list);
								var temp_trunk = [];
								this.egress_trunk_list.forEach(function(trunk, i){
									var temp = {};
									temp.id = trunk.trunk_id;
									temp.text = trunk.name;
									temp_trunk.push(temp);
								});
								var vm = this;
								this.egress_trunk_list.forEach(function(trunk, i){
									vm.egresses.push({
										selected: false,
										carrier: trunk.carrier_id,
										trunk: trunk.trunk_id,
										trunk_list: temp_trunk,
									});
								});
							}
						})
			...
			getDynamicRoutingForRequest() {
				const dynamic_routing_template = JSON.parse( JSON.stringify( this.dynamic_routing_template ) )

					var vm = [];
					if (this.egresses.length > 0) {
						this.egresses.forEach(function(egress, i){
							var temp = {
								resource_id: egress.trunk
							}
							vm.push(temp);
						});
					}
...
@media only screen and (max-width: 768px) and (min-width: 425px){
	...
	@media (max-width: 505px){
		#app > div.container-fluid > div > div > div > form > div.table-responsive.m-top-10 > table > tbody > tr > td:nth-child(2) > select, #app > div.container-fluid > div > div > div > form > div.table-responsive.m-top-10 > table > tbody > tr > td:nth-child(3) > select{
