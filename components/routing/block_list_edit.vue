				...
				<div class="white_pad">
					<form class="form" @submit.prevent="validateBeforeSubmit">
						...
						<div class="button_set little-space">
							...
							<button class="btn btn-default mini" v-on:click="reset">
								Reset
							</button>
						</div>
					</form>
	...
	module.exports = {
		...
		created: function(){
			...
			this.$http.get(api.getEndpointUrl() + "/v1/route/block/" + id, 
			{
				...
				this.origin_data = JSON.parse( JSON.stringify( this.data ) );
		...
		methods: {
			...
			reset : function(e){
				e.preventDefault();
				this.data = JSON.parse( JSON.stringify( this.origin_data ) )
				...
				this.$validator.validateAll().then(() => {
					...
					},
					{
						...
						function(response) {
							...
							console.log(response);
							this.setMessage({
								message: "Changing route block is failed!",
								type: 'error'
							});
						}
					)
