<template>
    <div class="container-fluid">
                            ...
                            <table class="table table-striped table-hover aliases">
                                <thead>
                                    <tr>
                                        ...
                                        <th v-if="showcolumns[3]">Trunk List</th>
                                        <th v-if="showcolumns[4]">Updated At</th>
                                ...
                                <tbody>
                                    ...
                                    <tr v-for="list in filterBy(lists, code)">
                                        ...
                                        <td v-if="showcolumns[5]">
                                            ...
                                            <a class="action action_delete" data-toggle="tooltip" data-placement="top" title="Delete" v-on:click="deleteRoute(list.item_id)">
                        ...
                        <div class="row">
                            <div class="col-md-3 col-sm-4">
                                <div class="form-group full-width-select">
                                    ...
                                    <select2 :options="options_data_format" v-model="data_format"></select2>
                                </div>
                            ...
                            <div class="col-md-3 col-sm-3">
                                <div class="form-group full-width-select">
                                    ...
                                    <select2 :options="options_export_group" v-model="column_1"></select2>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-3">
                                <div class="form-group full-width-select">
                                    <label for="username">Column #2:</label>
                                    <select2 :options="options_export_group" v-model="column_2"></select2>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-3">
                                <div class="form-group full-width-select">
                                    <label for="username">Column #3:</label>
                                    <select2 :options="options_export_group" v-model="column_3"></select2>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-3">
                                <div class="form-group full-width-select">
                                    <label for="username">Column #4:</label>
                                    <select2 :options="options_export_group" v-model="column_4"></select2>
                                </div>
                            </div>
                            <div class="col-md-3 col-sm-3">
                                <div class="form-group full-width-select">
                                    <label for="username">Column #5:</label>
                                    <select2 :options="options_export_group" v-model="column_5"></select2>
                                </div>
                            </div>
                            <!-- <div class="col-md-3 col-sm-3">
                                <div class="form-group full-width-select">
        ...
        <modal v-if="showRouteModal=='add_route' || showRouteModal=='edit_route'" @close="showRouteModal = ''" class="big_modal">
            <h3 slot="header" v-if="showRouteModal=='add_route'">Add New Route</h3>
            <h3 slot="header" v-if="showRouteModal=='edit_route'">Edit Route</h3>
            <div slot="body">
                <form class="form">
                    <div class="row">
                    ...
                    <div>
                        <a class="btn btn-primary submit mini" @click="validateBeforeSubmit">Submit</a>
                        <a @click="showRouteModal = ''" class="btn btn-default cancel mini">Cancel</a>
                    </div>
                </form>
            </div>
            <div slot="footer">
...
module.exports = {
    ...
    data: function() {
        return {
            ...
            options_export_group: [
                { id: '0', text: '' },
                { id: '1', text: 'Code' },
                { id: '2', text: 'Strategy' },
                { id: '3', text: 'Trunk name' },
                { id: '4', text: 'Percentage' },
                { id: '5', text: 'Time profile name' }
            ],
            file_breakdown: 2,
            file_options: [
    ...
    methods: {
        ...
        fetchRoutes() {
            ...
            const url = api.getEndpointUrl() + '/v1/route/static/' + id + '/item/list' + "?page=" + page + "&per_page=" + per_page + "&order_by=item_id&order_dir=desc"
            console.log(url);
            this.$http.get(url,
                {
                    headers: {
                    ...
                    this.lists.forEach(function(item, i) {
                        var list = self.getTrunkNameList(item.trunks);
                        console.log(list);
                        Vue.set(item, 'trunk_list', list);
                    });
        ...
        saveRoute(trunks) {
            var reqBody = {};
            reqBody.static_route_id = this.static_rouitng_id;
            reqBody.strategy = this.strategy;
            reqBody.digits = this.code_name;
            if (this.time_profile != 0)
                reqBody.time_profile_id = this.time_profile;
            reqBody.trunks = trunks;
            if (this.showRouteModal == "add_route") {

                this.$http.post(api.getEndpointUrl() + '/v1/route/static/' + this.static_rouitng_id + '/item/create', reqBody)
                    .then(
                    function(response) {
                        this.loading = false;
                        this.closeModal();
                        ...
                        var self = this;
                        this.setMessage('The route has been saved!');
                        setTimeout(function(){
            ...
            } else if (this.showRouteModal == "edit_route") {
                this.$http.patch(api.getEndpointUrl() + '/v1/route/static/item/' + this.routeid, reqBody)
                    .then(
                    function(response) {
                        this.loading = false;
                        this.closeModal();
        ...
        validateBeforeSubmit() {
            ...
            this.$validator.validateAll().then(() => {
                ...
                this.templateList.forEach(function(template, index) {
                    if (template.carrier == 0) {
                        vm.setMessage({
                            message: 'Carrier can not be empty!',
                            type: 'error'
                        })
                        isValidOk = false;
                        return;
                    ...
                    if (template.trunk == 0) {
                        vm.setMessage({
                            message: 'Trunk can not be empty!',
                            type: 'error'
                        })
                        isValidOk = false;
                        return;
        ...
        handleChange() {
            ...
            if (_.indexOf(this.route_table_columns, 'Updated At') != -1) {
        ...
        editRoute: function(route) {
            ...
            route.trunks.forEach(function(item, i) {
                var temp = {};
                let index = _.findIndex(self.carrier_lists, elem => elem.text === item.carrier_name)
                if(~index) {
                    temp.carrier = self.carrier_lists[index].id;
                    temp.trunk_lists = self.fetchTrunks(temp.carrier);
                }
            ...
            this.showRouteModal = 'edit_route';
        },
        addRoute: function() {
            this.code_name = '',
                this.time_profile = 0,
