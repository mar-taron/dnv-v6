<template>
	<div class="container-fluid">
					...
					<table class="table table-striped table-hover aliases">
						<thead>
							<tr role="row">
								...
								<th v-if="showcolumns[0]" @click="sortColumn('name')">Name<span class="dnl_icon dnl_sort pull-right"></span></th>
								<th v-if="showcolumns[1]" @click="sortColumn('defined_by')">Template Name<span class="dnl_icon dnl_sort pull-right"></span></th>
								<th v-if="showcolumns[2]" @click="sortColumn('code_deck_name')">Code Deck<span class="dnl_icon dnl_sort pull-right"></span></th>
								<th v-if="showcolumns[3]" @click="sortColumn('routed_by')">Route By<span class="dnl_icon dnl_sort pull-right"></span></th>
								<th v-if="showcolumns[4]" @click="sortColumn('update_at')">Updated At<span class="dnl_icon dnl_sort pull-right"></span></th></th>
								<th v-if="showcolumns[5]" @click="sortColumn('prefix_count')">Prefix Count<span class="dnl_icon dnl_sort pull-right"></span></th>
								<th v-if="showcolumns[6]" @click="sortColumn('route_count')">Route Count<span class="dnl_icon dnl_sort pull-right"></span></th>
								<th v-if="showcolumns[7]" @click="sortColumn('update_by')">Updated By<span class="dnl_icon dnl_sort pull-right"></span></th>
						...
						<tbody>
							...
							<tr v-for="(routing,index) in filterBy(routings, search)">
								...
								<td v-if="showcolumns[8]">
									...
									<a class="action action_copy" data-toggle="tooltip" data-placement="top" title="Copy" @click="showCopyRouting(routing)">
		...
		<h3 slot="header">Copy Routing</h3>
			<div slot="body">
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group" style="text-align: left">
							<label>Name:</label>
							<input type="text" name="copy_routing_name" v-model="copy_routing_name" class="form-control" placeholder="Enter Name">
						</div>
					</div>
				</div>
		  	</div>
			<div slot="footer">
				<button class="btn btn-primary submit mini" @click="copyRouting">Submit</button>
				<button @click="closeModal" class="btn btn-default cancel mini">Cancel</button>
			</div>
	  </modal>
	</div>
...
module.exports = {
	...
	data: function() {
		return {
			...
			routing_table_columns: ['Name', 'Defined By', 'Code Deck',
				'Route By', 'Updated At', 'Prefix Count', 'Route Count', 'Updated By', 'Action'],
	...
	methods: {
		...
		copyRouting() {
			console.log("copyRouting");
			this.loading = true;
			let index = _.findIndex(this.code_deck_list, elem => elem.text   === this.current_routing.code_deck_name)
			var reqBody = {};
			reqBody.name = this.copy_routing_name;
			if(~index)
				reqBody.code_deck_id = this.code_deck_list[index].id;
			reqBody.routed_by = this.current_routing.routed_by;
			reqBody.defined_by = this.current_routing.defined_by;
			this.$http.post(api.getEndpointUrl() + '/v1/route/static/create', reqBody)
			.then(
				function(response) {
					this.loading = false;
					console.log(response);
					this.showModal = '';
					this.setMessage('The static routing has been copied!');
				...
				function(response) {
					...
					this.showModal = '';
					this.setMessage({
						message:"Copying static routing is failed!",
						type:"error"
					});
				}
			)
		},
		showCopyRouting(routing) {
			this.showModal = 'copy';
			this.current_routing = routing;
			console.log(this.current_routing);
		},
        sortColumn(column) {
