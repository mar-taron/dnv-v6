...
<script>   
	...
	module.exports = {
		created: function(){
			...
			this.$http.get(api.getEndpointUrl() + "/v1/switch/code_deck/list", 
			{
				...
				this.$http.get(api.getEndpointUrl() + this.apiUrl + id,
				{
					...
					Vue.set(this.data, 'code_deck_name', routing.code_deck_name);

					let index = _.findIndex(this.code_deck_list, elem => elem.text   === routing.code_deck_name)
					if(~index)
						Vue.set(this.data, 'code_deck', this.code_deck_list[index].id);

					if(this.data.defiled_by == 'Code')
		...
		watch: {
			'defined_by': function () {
				if(this.data.defined_by == 'Code'){
					this.show_codedeck = false;
					this.data.code_deck = 0;
				}
				else {
					this.show_codedeck = true;
					this.data.code_deck = 0;
		...
		data: function(){
			...
			//   name: '',
			  defined_by_list: [
				  ...
				  { id: 'Code Name', value: 'Code Name', text: 'Code Name' },
