				...
				<div class="white_pad table_wrap">
					<div class="table_filters searcheable">
						<div class="inline-block search_wrapper">
							<input type="text" name="role_search" v-model="role_search" class="form-control inline-block search_field" placeholder="Query" v-on:keyup.enter="search">
					...
					<div class="table-responsive">
						<table class="table table-striped table-hover roles">
							<thead>
								<tr>
									<th @click="sortColumn('role_name')">Roles
										<span class="dnl_icon dnl_sort pull-right"></span>
									</th>
									<th @click="sortColumn('user_count')" class="width_138">User Count
										<span class="dnl_icon dnl_sort pull-right"></span>
									</th>
	...
	module.exports = {
		...
		methods: {
			...
			search() {
                ...
                this.order = ''
				this.sort = ''
				let url = api.getEndpointUrl() + '/v1/config/role/list' + this.getQueries()
				this.$http.get(url)
					.then(response => {
						this.loading = false
						...
						if (body.success) {
							...
							this.pageOne.totalPages = Math.ceil(payload.total / this.pageOne.cntpage)
						}
					})
					.catch(error => {
						this.loading = false
            ...
            getQueries($join = '?') {
                const role_name = 'role_name=' + this.role_search
                ...
                return $join + role_name + page + per_page + order
            },
			fetchRoles() {
				this.loading = true
				let url = api.getEndpointUrl() + '/v1/config/role/list' + this.getQueries()
