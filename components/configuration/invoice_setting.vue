				...
				<h3 class="description">You may specify how your Invoice should be generated</h3>
					...
					<div class="col-sm-9">
						...
						<section class="config_section">
							...
							<ckeditor v-model="contentA" :id="editorA" :height="'200px'"></ckeditor>
							...
							<ckeditor v-model="contentB" :id="editorB" :height="'200px'"></ckeditor>
							 <!-- <el-input type="textarea" :autosize="{ minRows: 2, maxRows: 4}" placeholder="Please input" v-model="contentB"> 
                            </el-input> -->
						</section>
...
<script>   
	...
	const Dropzone = require('vue!../main_components/dropzone.vue'),
	...
	module.exports = {
		...
		methods: {
			...
			reset() {
				this.invoice = JSON.parse( JSON.stringify( this.bakup_invoice ) )
			},
			getInvoiceForRequest () {
				const invoice_template = JSON.parse( JSON.stringify( this.invoice_template ) )

				invoice_template.decimal = this.invoice.decimal;
				invoice_template.billing_info = this.contentA;
				...
				invoice_template.company_info = this.contentB;
			...
			fetchInvoice () {
				...
				.then(response => {
					...
					if (body.success) {
						...
						this.contentA = invoice.billing_info;
						this.contentB = invoice.company_info;
						console.log(this.invoice);

						this.bakup_invoice = JSON.parse( JSON.stringify( this.invoice ) )
