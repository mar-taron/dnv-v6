	...
	module.exports = {
		created () {
			...
			.then(function(response) {
				if (response.body.success) {
					...
					this.pass = '*';
					if(email_sender.enable_auth != null)
						this.authen = email_sender.enable_auth ? 1 : 2
      ...
      return {
		...
		methods: {
			...
			getRequestForMailSender() {
				var mail_sender = {};
				mail_sender.email = this.emailaddress;
				mail_sender.name = this.sendername;
				mail_sender.password = this.pass;
				if(this.authen != 0) {
					mail_sender.enable_auth = this.authen == 1? true: false;
				}
				mail_sender.mail_server = this.mail_server;
				mail_sender.mail_port = this.mail_port;
				mail_sender.username = this.username;
				return mail_sender;
			},
			validateBeforeSubmit () {
				...
				.then(() => {
					...
					this.loading = true;
					const reqBody = this.getRequestForMailSender();
					this.$http.patch(api.getEndpointUrl() + '/v1/config/mail_sender/' + id, reqBody)
					.then(response => {
						if (response.body.success) {
							this.setMessage('Email sender was edited successfully');
							var thisvm = this;
							setTimeout(function(){
								thisvm.$router.push('/configuration/email_sender');
							}, 3000);
						}
						this.loading = false;
					})
					.catch(error => {
						...
						console.log(error)
						this.setMessage({
							message: 'Failed to change Email sender',
							type: 'error'
						})
					})
				}).catch(() => { })
