				...
				<div class="white_pad table_wrap">
					...
					<div class="table-responsive">
						<table class="table table-striped table-hover aliases credits">
							<thead>
								<tr>
									<th @click="sortColumn('username')" v-if="showcolumns[0]">Username
										<span class="dnl_icon dnl_sort pull-right"></span>
									</th>
									<th  @click="sortColumn('main_email')" v-if="showcolumns[1]">Main E-mail Address
										<span class="dnl_icon dnl_sort pull-right"></span>

									</th>
									<th  @click="sortColumn('company')" v-if="showcolumns[2]">Company Name
										<span class="dnl_icon dnl_sort pull-right"></span>

									</th>
									<th  @click="sortColumn('referral')" v-if="showcolumns[3]">Referral
										<span class="dnl_icon dnl_sort pull-right"></span>

									</th>
									<th  @click="sortColumn('client_name')" v-if="showcolumns[4]">Client Name
										<span class="dnl_icon dnl_sort pull-right"></span>

									</th>
									<th  @click="sortColumn('phone')" v-if="showcolumns[5]">Phone Number
										<span class="dnl_icon dnl_sort pull-right"></span>

									</th>
									<th  @click="sortColumn('signed_up_on')" v-if="showcolumns[6]">Registration Time
										<span class="dnl_icon dnl_sort pull-right"></span>
									</th>
									<th  @click="sortColumn('modified_on')" v-if="showcolumns[7]">Operation Time
										<span class="dnl_icon dnl_sort pull-right"></span>
									</th>
									<th  @click="sortColumn('status')" v-if="showcolumns[8]">Status
										<span class="dnl_icon dnl_sort pull-right"></span>
									</th>
							...
							<tbody>
								...
								<tr v-for="user in filterBy(users, reg_time)">
									...
									<td v-if="showcolumns[6]">
										{{user.signed_up_on.substr(0, 10) + ' ' + user.signed_up_on.substr(11, 8) + user.signed_up_on.substr(26, 3)}}
									</td>
									<td v-if="showcolumns[7]">
										{{user.modified_on.substr(0, 10) + ' ' + user.modified_on.substr(11, 8) + user.modified_on.substr(26, 3)}}
									</td>
									<td v-if="showcolumns[8]">{{user.status}}</td>
									<td v-if="showcolumns[9]">
										<a class="action action_disable" data-toggle="tooltip" data-placement="top" title="Approve" v-if="user.status!='approved' && user.status!='rejected'" v-on:click="approve_user(user.id)">
										...
										<a class="action action_reject" data-toggle="tooltip" data-placement="top" title="Reject" v-if="user.status!='approved' && user.status!='rejected'" v-on:click="reject_user(user.id)">
	...
	methods: {
		...
		submit: function () {
			...
			this.selected_ids.forEach(function (item, i) {
				...
				if (userid == 0) {
					vm.setMessage({
						message: 'The registration has been not selected!',
						type: 'error'
					});
				}
				else {
					vm.loading = true;
					vm.resource = vm.$resource(api.getEndpointUrl() + '/v1/registration/' + userid);
                         vm.resource.delete({ id: userid }).then(
                            function (response) {
                                vm.loading = false;
		...
		approve_user: function (id) {
			...
			this.$http.patch(api.getEndpointUrl() + '/v1/registration/' + id + '/approve',
				...
				function (response) {
					...
					this.setMessage('The user have been approved!')
				},
				function (response) {
						var thisvm = this;
						this.loading = false;
		...
		getQueries() {
			...
			if(this.start_date_time != '')
				query += '&signed_up_on_gt=' + this.start_date_time.toISOString()
			if(this.start_date_time != '')
				query += '&signed_up_on_lt=' + this.end_date_time.toISOString()
			if(this.status != 'all')
