      ...
      <div class="row">
		<div class="wrapper single">
			...
			<div class="btn-group" role="group">
			  <button type="button" class="btn btn-default" @click="sendAllLowBalance">
						...
						<tr>
						  <th class="width-65">
							  <div class="checkbox checkbox-success">
								<input type="checkbox" id="all_select" name="all_select" v-model="checker" @click="clickSelectedAll">
						  ...
						  <th v-if="showcolumns[9]">Updated at</th>
						  <th v-if="showcolumns[10]">Updated by</th>
					  ...
					  <tbody>
						...
						<tr v-for="credit in filterBy(credits, credit_search)">
						  <td class="centred_checkbox">
							  <div class="checkbox checkbox-success">
								<input :id="credit.client_id" :name="credit.client_id" type="checkbox" v-model="credit.selected">
						  ...
						  <td v-if="showcolumns[11]">
							  <a class="action action_test" data-toggle="tooltip" data-placement="top" title="Diactivate" @click="activeDeactive(credit.client_id, credit.active)">
							  ...
							  <a class="action action_copy" data-toggle="tooltip" data-placement="top" title="Send Low Balance Alert" @click="sendLowBalance(credit.client_id)">
	...
	module.exports = {
		...
		data: function(){
			return {
				...
				credit_mng_table_columns: ['Name', 'Credit Limit', 'CPS Limit', 'Call Limit', 'Terms', 'Payment Terms', 'Last Paid on', 'Last Payment', 'Warning Limit', 'Updated at', 'Updated by', 'Actions'],
				temp_credit_mng_table_columns: ['Name', 'Credit Limit', 'CPS Limit', 'Call Limit', 'Terms', 'Payment Terms', 'Last Paid on', 'Last Payment', 'Warning Limit', 'Updated at', 'Updated by', 'Actions'],
		...
		methods: {
			...
			sendAllLowBalance() {
				...
				this.credits.forEach(function (credit, i) {
					if (credit.selected) {
						var url = api.getEndpointUrl() + '/v1/carrier/' + id + '/send_low_balance_alert';
						this.$http.patch(url)
							.then(response => {
								if (response.body.success) {
									this.setMessage('Sending low balance alert was successfull');
								}
							})
							.catch(error => {
								console.log(error)
								this.setMessage({
									message: 'Failed to sending low balance alert',
									type: 'error'
								})
							})	
					}
			...
			activeAll() {
				...
				this.credits.forEach(function (credit, i) {
					if (!credit.active) {
						var url = api.getEndpointUrl() + '/v1/client/credit/' + credit.client_id;
						var reqBody = [];
						var action = 'activate';
						reqBody.active = true;
						...
						this.$http.patch(url, reqBody)
							.then(response => {
								if (response.body.success) {
									this.setMessage('Credit was ' + action + 'd successfully');
								}
							})
							.catch(error => {
								console.log(error)
								this.setMessage({
									message: 'Failed to ' + action + ' credit',
									type: 'error'
								})
							})	
					}
			...
			deActiveSel() {
				...
				this.credits.forEach(function (credit, i) {
					if (credit.selected & credit.acitve) {
						var url = api.getEndpointUrl() + '/v1/client/credit/' + credit.client_id;
						var reqBody = [];
						var action = 'deactivate';
						reqBody.acitve = false;
						console.log('Request Body:', reqBody);
						this.$http.patch(url, reqBody)
							.then(response => {
								if (response.body.success) {
			...
			activeDeactive(id, active_status) {
				var url = api.getEndpointUrl() + '/v1/client/credit/' + id;
				var reqBody = [];
				var action = 'activate';
