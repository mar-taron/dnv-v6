								...
								<table class="table table-striped table-hover carrier_groups originators">
									...
									<tbody>
										...
										<tr v-for="log in logs">
											<td v-if="showcolumns[0]">{{log.action_time}}</td>
...
module.exports = {
	...
	data: function () {
		return {
			...
			options: [
				{ id: 'All', text: 'All' },
				{ id: 'delete found rates', text: 'delete found rates' },
				{ id: 'insert as new rates', text: 'insert as new rates' },
				{ id: 'update current rates', text: 'update current rates' },
				{ id: 'update all rates', text: 'update all rates' }
			],
			selected1: 1,
			selected2: 1
		}
	...
	methods: {
		...
		query() {
			...
			const url = api.getEndpointUrl() + '/v1/log/rate_mass_edit' + this.getQueries()
				+ "&action_type=" + this.type + "&action_time_lt=" + this.time1 + "&action_time_gt=" + this.time2 + "&order_by=id&order_dir=desc";
			this.$http.get(url)
			.then(response => {
				const body = response.body
				if (body.success) {
