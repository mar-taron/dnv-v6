	...
	<div class="container-fluid">
		<div class="row">
			<div class="wrapper single">
				...
				<h1 class="page-header">Email Log</h1>
						...
						<div class="col-md-12">
							<div class="table-responsive marged-30">
								<table class="table table-striped table-hover carrier_groups originators">
									<thead>
										<tr>
											<th class="text-center" v-if="showcolumns[0]" @click="sortColumn('client_id')">Sent Time
												<span class="dnl_icon dnl_sort pull-right"></span>
											</th>
											<th v-if="showcolumns[1]" @click="sortColumn('client_id')">Carrier
												<span class="dnl_icon dnl_sort pull-right"></span>
											</th>
											<th v-if="showcolumns[2]" @click="sortColumn('type')">Type
												<span class="dnl_icon dnl_sort pull-right"></span>
											</th>
											<th v-if="showcolumns[3]" @click="sortColumn('sent_from')">E-mail Adress
												<span class="dnl_icon dnl_sort pull-right"></span>
											</th>
											<th v-if="showcolumns[4]" @click="sortColumn('status')">E-mail Status
												<span class="dnl_icon dnl_sort pull-right"></span>
											</th>
									...
									<tbody>
										...
										<tr v-for="log in logs">
											<td class="text-center" v-if="showcolumns[0]">{{log.date}}</td>
											<td class="text-center" v-if="showcolumns[1]">{{log.client_name}}</td>
											<td class="text-center" v-if="showcolumns[2]">{{log.type}}</td>
											<td class="text-center" v-if="showcolumns[3]">{{log.sent_from}}</td>
											<td class="text-center" v-if="showcolumns[4]">{{log.status}}</td>
...
module.exports = {
	...
	methods: {
		...
		query() {
			...
			this.loading = true;
			this.$http.get(api.getEndpointUrl() + this.apiUrl + "&type=" + this.type + "&date_lt=" + this.time_1 + "&date_gt=" + this.time_2 + this.getQueries(),
				{
					headers: {
...
<style>
	#app > div.container-fluid > div > div > div.timeline > div.chart_content > div.table_filters.marged-40 > div:nth-child(2){
		margin-left:30px;
	}
