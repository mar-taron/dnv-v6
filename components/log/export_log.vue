			...
			<div class="wrapper single">
				...
				<div class="timeline">
					<div class="chart_select_header grided">
						...
						<div class="col-md-3 col-sm-4">
							<a class="btn btn-primary m-top-10" v-on:click="runQuery()">
								Query
							</a>
						</div>
...
module.exports = {
	...
	methods: {
		...
		runQuery() {
			...
			var counter = parseInt(this.refresh) * 60000
			console.log(counter);
			this.refreshTimer = setInterval(this.query, counter)
		},
		query() {
			...
			var url = api.getEndpointUrl() + this.apiUrl + "?page=" + page + "&per_page=" + countperpage;
			if (this.download_time_1 != "") {
				url = url + "&download_time_lt=" + this.download_time_1.toISOString();
			}
			if (this.download_time_2 != "") {
				url = url + "&download_time_gt=" + this.download_time_2.toISOString();
			}
			url = url + "&order_by=id&order_dir=desc";
