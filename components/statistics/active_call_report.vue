				...
				<table class="table table-striped table-hover carrier_groups centrum">
				  <thead>
					<tr>						
						...
						<th colspan="1" rowspan="2">Routing Plan</th>													
					</tr>
					<tr>						
	...
	module.exports = {
		...
		methods: {
			...
			fetchIpLists (type){
				var url;
				if(type == 'egress')
					url = api.getEndpointUrl() + '/v1/trunk/ip/list?resource_id=' + this.egress_trunk;
				else if(type == 'ingress')
					url = api.getEndpointUrl() + '/v1/trunk/ip/list?resource_id=' + this.ingress_trunk;
				this.loading = true;
				this.$http.get(url)
				.then(response => {
					const body = response.body
					if (body.success) {
						const ips = body.payload.items
						var self = this;
						if(type == 'egress') {
						...
						this.ips.forEach(function (item, i) {
							...
							ip.text = value;								
							if(type == 'egress') {
								self.egress_trunk_ip_options.push(ip);
							}
							else if(type == 'ingress') {
								self.ingress_trunk_ip_options.push(ip);
							}
						});
						this.loading = false;
					}
