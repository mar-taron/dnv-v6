				...
				<tab label="Origination" class="" icon="dnl_icon dnl_to_left" selected>
					<div class="advanced_search_filter_panel">
						...
						<div class="col-lg-4 col-md-6 col-sm-6">
							<div class="white_pad less_pad">
								...
								<div class="row">
									<div class="col-lg-5 col-md-6 col-sm-6 first">
										<label>Carriers:</label>
									...
									<div class="col-lg-5 col-sm-6 col-md-6 col-sm-6 second">
					...
					<div class="table_filters pull-right">
						...
						<div class="inline-block">							
							<el-select v-model="show_hide_table_column" multiple placeholder="Show/Hide Columns" @change="handleChange">
								<el-option v-for="item in show_hide_table_column_options" :key="item.value" :label="item.text" :value="item.value">
								</el-option>
							</el-select>
						</div>
					</div>
					<span class="table_time chart_time margin_0">
						<p>Report Period 2017-09-07 19:00:02 +1000 - 2017-09-07 23:59:59 +1000</p>
					</span>
					<div class="clearfix"></div>
					<div class="table-responsive">
						<table class="table table-striped table-hover carrier_groups centrum">
						  <thead>
							<tr>								
								<th v-if="showcolumns[0]" rowspan="2" colspan="1">ASR</th>
								<th v-if="showcolumns[1]" rowspan="2" colspan="1">ACD(min)</th>								
								<th v-if="showcolumns[2]" rowspan="2" colspan="1">PDD(ms)</th>
								<th v-if="showcolumns[3]" rowspan="1" colspan="1">Time(min)</th>
								<th v-if="(showcolumns[4] && showcolumns[5] && showcolumns[6])"  rowspan="1" colspan="3">Calls</th>
								<th v-else-if="(((showcolumns[4] == false) || (showcolumns[5] == false) || (showcolumns[6] == false)) && !((showcolumns[4] == false) && (showcolumns[5] == false) && (showcolumns[6] == false)))" colspan="1">Calls</th>
							</tr>
							<tr>
								<th v-if="showcolumns[3]" rowspan="1" colspan="1">Total Billable Time</th>
								<th v-if="showcolumns[4]" rowspan="1" colspan="1">Total Calls</th>
								<th v-if="showcolumns[5]" rowspan="1" colspan="1">Not Zero</th>
								<th v-if="showcolumns[6]" rowspan="1" colspan="1">Busy Calls</th>								
							</tr>
						  </thead>
						  <tbody>
							...
							<tr v-for="origination in originations">							  
							  <td v-if="showcolumns[0]" >{{origination.asr}}</td>
							  <td v-if="showcolumns[1]" >{{origination.acd}}</td>							  
							  <td v-if="showcolumns[2]" >{{origination.pdd}}</td>
							  <td v-if="showcolumns[3]" >{{origination.egress_billed_time}}</td>
							  <td v-if="showcolumns[4]" >{{origination.egress_calls}}</td>
							  <td v-if="showcolumns[5]" >{{origination.non_zero_calls}}</td>							  
							  <td v-if="showcolumns[6]" >{{origination.egress_busy_calls}}</td>							  
							</tr>
						  </tbody>
						</table>
					</div>
	...
	module.exports = {
		...
		data: function(){
            return {
				...
				temp_show_hide_table_column: ['ASR','ACD(min)','PDD(ms)','Total Billable Time','Total Calls','Not Zero','Busy Calls'],

				show_hide_table_column: ['ASR','ACD(min)','PDD(ms)','Total Billable Time','Total Calls','Not Zero','Busy Calls'],

				show_hide_table_column_options: [{
				value: 'ASR',
				label: 'ASR'
			}, {
				value: 'ACD(min)',
		...
		methods: {
			...
			handleChange() {
						for (let i = 0; i < this.temp_show_hide_table_column.length; i++) {
				if (_.indexOf(this.show_hide_table_column, this.temp_show_hide_table_column[i]) != -1) {
