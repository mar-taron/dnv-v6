						...
						<section class="role_section basic_section">
                            ...
                            <div class="col-md-4 col-sm-6">
								<div class="form-group">
									...
									<input type="text" name="min_duration" v-model="template.min_duration" class="form-control" placeholder="Enter Min Duration">
								</div>
							</div>
                            <div class="col-md-4 col-sm-6">
								<div class="form-group">
	...
	module.exports = {
		...
		methods: {
			...
			getEgressTrunkTemplateForRequest() {
				const egress_trunk_template = JSON.parse( JSON.stringify( this.egress_trunk_template ) )
				...
				if(this.template.rpid_screen != '' && this.template.rpid_screen != null)
				...
				if(this.template.multi_host != '' && this.template.res_strategy != null)
				...
				if(this.template.media_type != '' && this.template.media_type != null)
				...
				egress_trunk_template.min_duration = this.template.min_duration;

				return egress_trunk_template;
			},
			saveEgressTrunkTemplate() {
				const reqBody = this.getEgressTrunkTemplateForRequest();
				var url;
				if(this.editingtemplate) {
					url = api.getEndpointUrl() + '/v1/config/egress_trunk_template/' + this.id;
					this.$http.patch(url, reqBody)
						.then(response => {
							if (response.body.success) {
								this.setMessage('Egress trunk template was changed successfully');
								var thisvm = this;
								setTimeout(function(){
						...
						.catch(error => {
							console.log(error)
							this.setMessage({
								message: 'Failed to change Egress trunk template',
								type: 'error'
							})
						})
				}
				else {
					url = api.getEndpointUrl() + '/v1/config/egress_trunk_template/create';
