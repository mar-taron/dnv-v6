            ...
            <div class="white_pad table_wrap">
                <div class="advanced_search_filter_panel bottom_0">
					...
					<div class="inline-block search_wrapper">
						...
						<a class="btn btn-primary" @click="searchByName">
							Query
						</a>
					</div>
                </div>
                ...
                <div class="table_filters bottom_0">
                    ...
                    <div class="inline-block">
                        <el-select v-model="ingress_temp_table_columns" multiple placeholder="Show/Hide Columns" @change="handleChange">
                            <el-option v-for="item in ingress_temp_table_column_options" :key="item.value" :label="item.label" :value="item.value">
                            </el-option>
                        </el-select>
                    </div>
                </div>
	...
	module.exports = {
		...
		data: function(){
            return {
                ...
                back_ingress_temp_table_columns: ['Template Name', 'Created on', 'Created by', 'Last Updated', 'Used by', 'Action'],
                ingress_temp_table_columns: ['Template Name', 'Created on', 'Created by', 'Last Updated', 'Used by', 'Action'],
                ingress_temp_table_column_options: [{
						value: 'Template Name',
						label: 'Template Name'
					},
					{
		...
		methods: {
			...
			searchByName() {
				...
				const per_page = this.pageOne.per_page;
				const orderby = 'update_on&order_dir=desc'
				let url = api.getEndpointUrl() + '/v1/config/ingress_trunk_template/list?name=' + this.template_name
				url += '&page=' + page
            ...
            handleChange() {
				for(let i = 0; i < this.back_ingress_temp_table_columns.length; i++) {
					if (_.indexOf(this.ingress_temp_table_columns, this.back_ingress_temp_table_columns[i]) != -1) {
            ...
            fetchIngressTrunkTemplate () {
                const url = api.getEndpointUrl() + '/v1/config/ingress_trunk_template/list' + this.getQueries()
				this.$http.get(url)
				.then(response => {
					const body = response.body
					if (body.success) {
			...
			pageOneChanged (pageNum) {
				if (pageNum !== this.pageOne.currentPage) {
					...
					this.$nextTick(this.fetchIngressTrunkTemplate)
				}
			},
            closeModal: function() {
				this.showModal = '';
