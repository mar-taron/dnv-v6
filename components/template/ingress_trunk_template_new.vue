				...
				<tabs class="vertical">
						...
						<section class="role_section">
							...
							<div class="col-md-4 col-sm-6">
								<div class="form-group">
									...
									<input type="text" name="template_name" v-model="template.template_name" class="form-control" :class="{'input': true, 'is-danger': errors.has('template_name') }" v-validate="'required'" placeholder="Template Name">
					...
					<tab label="Trunk Info" class="" icon="dnl_icon dnl_arrow-point-to-right">
						<section class="role_section basic_section">
                            ...
                            <div class="col-md-4 col-sm-6">
								<div class="form-group">
									<label for="pdd_timeout">PDD Timeout(ms):</label>
                            ...
                            <div class="col-md-4 col-sm-6">
								<div class="form-group">
									<label for="ring_timer">Ring Timer(s):</label>
									<input type="text" name="ring_timer" v-model="template.ring_timer" class="form-control" :class="{'input': true, 'is-danger': errors.has('ring_timer') }" v-validate="'required'" placeholder="Enter Ring Timer">
                            ...
                            <div class="col-md-4 col-sm-6">
								<div class="form-group fix-select">
									...
									<select2 :options="ignore_early_media_options" v-model="template.ignore_early_media">
									</select2>
								</div>
							</div>
							<div class="clearfix little-space"></div>
                                   ...
                                   <tbody>
									   <template v-for="(item, index) in actions">
										<tr>
											...
											<td>
												<input type="text" v-model="item.number_length" class="form-control width-80" :disabled="item.number_type=='all'">
											</td>
											<td>
                                   ...
                                   <tbody>
									   <template v-for="(item, index) in fail_rules">
										<tr>
											...
											<td>
												<input type="text" name="code" v-model="item.code" class="form-control" :class="{'input': true, 'is-danger': errors.has('code') }" v-validate="'required|numeric'" placeholder="">
												<span v-show="errors.has('code')" class="help is-danger">{{ errors.first('code') }}</span>
											</td>
											<td>
												<div class="row">
													<div class="col-md-6">
														<input type="text" v-model="item.response_1" class="form-control">
													</div>
													<div class="col-md-6">
														<input type="text" v-model="item.response_2" class="form-control">
													</div>
												</div>
											</td>
											<td>
						...
						<section class="role_section">
                            ...
                            <div class="col-md-4 col-sm-6">
								<div class="form-group">
									...
									<select class="selectable" v-model="template.is_display_name">
										<option v-for="item in display_name_options" v-bind:value="item.id">
											{{ item.text }}
										</option>
									</select>
								</div>
	...
	module.exports = {
		...
		data: function(){
            return {
				...
				bill_by_options: [
                    {id: 0, text: 'LRN'},
                    {id: 1, text: 'LRN BLOCK'},
                    {id: 2, text: 'LRN BLOCK HIGHER'},
                    {id: 3, text: 'DNIS'},
                    {id: 4, text: 'FLOW RATE DECK'},
                ],
				ignore_early_nosdp: true,
				ignore_early_media_options: [
					{id: '0', text: 'None'},
					{id: '1', text: '180 and 183'},
					{id: '2', text: '180 only'},
					{id: '3', text: '183 only'},
				],
				ignore_options: [
		...
		methods: {
			...
			getIngressTrunkTemplateForRequest() {
				const ingress_trunk_template = JSON.parse( JSON.stringify( this.ingress_trunk_template ) )

				ingress_trunk_template.profit_margin = 0;									// not exist in UI
				ingress_trunk_template.lnp_dipping_rate = 0;									// not exist in UI
				if(this.template.rpid_party != null)
				...
				if(this.template.rpid_privacy != null)
				...
				if(this.template.rpid_screen != null)
				...
				if(this.template.multi_host != null)
				...
				if(this.template.media_type != null)
				...
				if(this.template.rpid_id_type != null)
			...
			saveIngressTrunkTemplate() {
				const reqBody = this.getIngressTrunkTemplateForRequest();
				var url;
				if(this.editingtemplate) {
					url = api.getEndpointUrl() + '/v1/config/ingress_trunk_template/' + this.id;
					this.$http.patch(url, reqBody)
						.then(response => {
							if (response.body.success) {
								this.setMessage('Ingress trunk template was changed successfully');
								var thisvm = this;
								setTimeout(function(){
						...
						.catch(error => {
							console.log(error)
							this.setMessage({
								message: 'Failed to change Ingress trunk template',
								type: 'error'
							})
						})
				}
				else {
					url = api.getEndpointUrl() + '/v1/config/ingress_trunk_template/create';
