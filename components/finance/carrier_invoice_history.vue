					...
					<table class="table table-striped table-hover roles carriers">
					  <thead>
						<tr>
						  ...
						  <th v-if="showcolumns[6]" class="width-138">Action</th>
						</tr>
					  </thead>
					  <tbody>
						...
						<tr v-for="carrier in carriers">
						  <td v-if="showcolumns[0]">{{carrier.pt_name}}</td>
						  <td v-if="showcolumns[1]">{{carrier.payment_term_id}}</td>
							<td v-if="showcolumns[2]"></td>
							<td v-if="showcolumns[3]">{{carrier.last_invoice != null? carrier.last_invoice.amount: ''}}</td>
							<td v-if="showcolumns[4]">{{carrier.last_invoice != null? carrier.last_invoice.invoice_period_from: ''}}</td>
							<td v-if="showcolumns[5]">{{carrier.last_invoice != null? carrier.last_invoice.invoice_date: ''}}</td>
						  <td v-if="showcolumns[6]">
								<a class="action action_download" data-toggle="tooltip" data-placement="top" title="Download Latest Invoice"><span class="dnl_icon dnl_download"></span></a>
								...
								<a class="action action_edit" data-toggle="tooltip" data-placement="top" title="Edit Auto-Invoice Config"><span class="dnl_icon dnl_action_edit"></span></a>
	...
	module.exports = {
		...
		methods: {
			searchByName() {
				...
				let url = api.getEndpointUrl() + '/v1/finance/auto_invoice/list?pt_name=' + this.carrier_search
				url += '&page=' + page
