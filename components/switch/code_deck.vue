						...
						<table class="table table-striped table-hover agents">
							<thead>
								<tr>
									...
									<th class="table_column" @click="sortName" v-if="showcolumns[0]">Name
										<span class="dnl_icon dnl_sort pull-right"></span>
									</th>
									<th class="table_column" @click="sortCodeCnt" v-if="showcolumns[1]">Code Count<span class="dnl_icon dnl_sort pull-right"></span></th>
									<th class="table_column" @click="sortUsageCnt" v-if="showcolumns[2]">Usage Count<span class="dnl_icon dnl_sort pull-right"></span></th>
									<th v-if="showcolumns[3]">Updated at</th>
									<th v-if="showcolumns[4]">Updated by</th>
...
module.exports = {
	...
	methods: {
		...
		sortCodeCnt() {
			if(this.code_cnt_sort == 'asc') {
				this.code_cnt_sort = 'desc'
				this.sort = this.code_cnt_sort
				this.order = 'codes'
				this.fetchDecks()
			}
				
			else if(this.code_cnt_sort == 'desc') {
				this.code_cnt_sort = 'asc'
				this.sort = this.code_cnt_sort
				this.order = 'codes'
				this.fetchDecks()
			}
		...
		sortUsageCnt() {
			if(this.usage_cnt_sort == 'asc') {
				this.usage_cnt_sort = 'desc'
				this.sort = this.usage_cnt_sort
				this.order = 'usage_count'
				this.fetchDecks()
			}
				
			else if(this.usage_cnt_sort == 'desc') {
				this.usage_cnt_sort = 'asc'
				this.sort = this.usage_cnt_sort
				this.order = 'usage_count'
				this.fetchDecks()
			}
		...
		fetchDecks() {
			...
			this.$http.get(api.getEndpointUrl() + '/v1/switch/code_deck/list' + "?page=" + page + "&per_page=" + per_page + "&order_by=" + this.order + "&order_dir=" + this.sort,
				{
					headers: {
		...
		handleChange() {
			...
			if (_.indexOf(this.deck_table_columns, 'Updated at') != -1) {
			...
			if (_.indexOf(this.deck_table_columns, 'Updated by') != -1) {
		...
		createNew: function() {
			this.$router.push({path:"/switch/code_deck/new"});
		},
		deleteAll: function () {
			this.selected_ids = [];
			var vm = this;
