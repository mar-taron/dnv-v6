				...
				<div class="white_pad">
					...
					<form class="form">
						...
						<div role="group" class="btn-group"> 
							...
							<button type="button" class="btn btn-default remove_icon" @click="removeSelected">
								<span class="dnl_icon dnl_action_delete"></span> Delete Selected
							</button> 
							<button type="button" class="btn btn-default remove_icon" @click="removeAll">
									...
									<template v-for="(rate, index) in rates">
										...
										<tr v-if="rate.show_extra_field">
											<td colspan="11">
												...
												<div class="col-md-2">
													<label for="interval">Grace Time:</label>
													<input type="text" name="" class="form-control" placeholder="Grace Time">
												</div>
												<div class="col-md-2">
													<label for="interval">Sec Seconds:</label>
													<input type="text" name="" class="form-control" placeholder="Sec Seconds">
												</div>
												<div class="col-md-2">
	...
	module.exports = {
		...
		data: function(){
			return {
			  ...
			  types: [
				  ...
				  { id: 'LRN BLOCK', value: 'LRN BLOCK', text: 'LRN BLOCK' },
			  ],
			  needType: false,
		...
		watch: {
			'rate_type': function() {
				...
				if(this.rate_type == 'US Jurisdictional' || this.rate_type == 'US Non Jurisdictional') {
					this.needType = true;
					this.type = 'DNIS';
				}
				else {
			...
			saveRateTable() {
				...
				requestForm,
				{
					...
					if (response.body.success) {
						...
						this.created_object_id = response.body.object_id;
						...
						if (this.rates.length > 0) {
							...
							},
							{
								...
								if (response.body.success) {
									...
									this.$router.push('/switch/rate_table/editing_rates/' + this.created_object_id);
...
<style>
	...
	.btn-group .btn.btn-default.remove_icon {
		padding-top: 9px;
	}
</style>
