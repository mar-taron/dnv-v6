...
module.exports = {
	...
	methods: {
		...
		getCodeDeckForRequest() {
			const code_template = JSON.parse( JSON.stringify( this.code_template ) )
			// code_template.codes = this.code_deck.codes;
			code_template.codes = [];
			console.log(code_template.codes);
			if(this.editingcode) {
				console.log(this.code);
				
				var code = {};
				code.code = this.code.code;
				code.code_name = this.code.code_name;
				code.country = this.code.country;
				if(this.code.state != null)
					code.state = this.code.state;
				if(this.code.city != null)
					code.city = this.code.city;
				code_template.codes[this.index] = code;
			}
			else {
				// code_template.state= this.code.state;
				code_template.name = this.code.code_name;
				code_template.code = this.code.code;
				// code_template.city = this.code.city;
				code_template.code_deck_id = this.id;
				code_template.country = this.code.country;
			}
		...
		saveCodeDeck() {
			...
			var url = api.getEndpointUrl() + '/v1/switch/code/create';
			if (this.editingcode) {
				url = api.getEndpointUrl() + '/v1/switch/code_deck/' + this.code_deck.code_deck_id;
				this.$http.patch(url, reqBody)
				.then(response => {
					if (response.body.success) {
						this.$router.push('/switch/code_deck/code_deck_list/'+this.code_deck.code_deck_id);
						this.setMessage('Code was changed successfully');
					}
					this.loading = false;
				})
				.catch(error => {
