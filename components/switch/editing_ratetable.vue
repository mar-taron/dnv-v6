			...
			fetchRateTable(id) {
				...
				.then(response => {
					...
					if (body.success) {
						...
						if (typeof this.ratetable.billing_method !== 'undefined')
							this.needType = true;
						else
							this.needType = false;
						if(this.ratetable.rate_type_name == 'A-Z')
							this.needType = false;
						else
							this.needType = true;
				...
				this.$validator.validateAll().then(() => {
					...
					requestForm, 
					{
						...
						function(response) {
							...
							this.$router.push('/switch/rate_table/editing_rates/'+this.$route.params.id);
						},
						function(response) {
							this.loading = false;
							console.log(response);
