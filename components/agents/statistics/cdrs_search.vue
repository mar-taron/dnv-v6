<template>
    <div style="margin-top: 140px!important;" class="container-fluid container-top-135">
        <div class="row">
            <div class="wrapper single">
                ...
                <div class="row">
                    <div class="col-md-2 col-sm-6 pad-right">
                        <a class="btn btn-primary full-width m-top-20 btn-width-90" v-on:click="searchReport()">
                            Download CDRs
                        </a>
                    </div>
                    <div class="col-md-2 col-sm-6 pad-left">
                        <a class="btn btn-primary full-width m-top-20 btn-width-90" v-on:click="searchReport()">
                            View Failed Calls
                        </a>
                    </div>
                ...
                <tabs>
                    <tab label="Successfull Calls(Latest 500 attempts)" class="" icon="dnl_icon dnl_qos_report" selected>
                        ...
                        <div class="clearfix little-space"></div>
                        <div class="row">
                            <div class="col-md-2 col-sm-6 trunk_select_container">
                                <label class="trunks">Trunks:</label>
                                <select2 class="trunks" :options="trunk_list" v-model="trunk">
                                    <option disabled value="0">All Trunks Group</option>
                                </select2>
                            </div>

                            <div style="margin-top: 35px" class="col-sm-5 text-center pull-right">
                            ...
                            <table class="table table-striped table-hover aliases">
                                <thead>
                                    <tr>
                                        ...
                                        <th v-if="showcolumns[6]">Number Type</th>
                                        <th v-if="showcolumns[7]">Pdd</th>
                                        <th v-if="showcolumns[8]">Trunk Group</th>
                                        <th v-if="showcolumns[9]">Hangup Cause</th>
                                        <th v-if="showcolumns[10]">Reason</th>
                    ...
                    <tab label="Successfull SMS(a) (Latest 500 attempts)" class="" icon="dnl_icon dnl_export">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover carrier_groups centrum">
                                <thead>
                                <tr>
                                    <th v-if="showcolumns[0]">Rule Name</th>
                                    <th v-if="showcolumns[1]">Start Time</th>
                                    <th v-if="showcolumns[2]">Finish Time</th>
                                    <th v-if="showcolumns[3]">Create by</th>
                                    <th v-if="showcolumns[4]">Status</th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr v-for="log in ex_logs">
                                    <td v-if="showcolumns[0]">{{log.rule_name}}</td>
                                    <td v-if="showcolumns[1]">{{log.start_time}}</td>
                                    <td v-if="showcolumns[2]">{{log.finish_time}}</td>
                                    <td v-if="showcolumns[3]">{{log.create_by}}</td>
...
#app > div.container-fluid > div > div > div.tabs > div.tabs_container > div:nth-child(1) > div.row > div.col-md-2.col-sm-6.trunk_select_container > span > span.selection > span > span.select2-selection__arrow {
         height: 36px;
         right: 8px;
         width: 27px;
         top: 10px;
     }
     #app > div.container-fluid > div > div {
         padding-top: 75px;
     }
