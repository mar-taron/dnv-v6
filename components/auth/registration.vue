		...
		<div class="container">
			<div class="reg_form_wrapper">
				<form class="form">
							...
							<section class="reg_form_section">
								...
								<template v-for="(item, index) in product_list">
									<!-- <div class="col-md-8 col-sm-8 col-xs-12">
										...
										<select class="selectable" v-model="item.name">
									...
									<div class="col-md-1 col-sm-2" v-if="index==0">
										<div class="inline_actions"  style="margin-top: 30px;">
											<a><span class="dnl_icon dnl_add" @click="addProduct"></span></a>
										</div>
									</div>
									<div class="col-md-1 col-sm-2" v-if="index>0">
					...
					<a class="btn btn-primary mini sign_up pull-right" type="button" v-on:click="register">Sign Up</a>
				</form>
			</div>
		</div>
	</div>
...
<script>
	...
	module.exports = {
		...
		watch: {
			'billing.same_as_base': function() {
				if(this.billing.same_as_base) {
					...
					this.billing.phone = this.user.phone;
					this.billing.email = this.company.main_email;
					this.billing.contact = this.company.business_contact;
				}
				else {
		...
		methods: {
			...
			// 		if (body.success) {
			// 			var products = body.payload.items;
			// 			var self = this;
			...
			getUserForRequest () {
				const user_template = JSON.parse( JSON.stringify( this.user_template ) );
